...
public class RoloClientConnector  {

	private static final Logger logger = LoggerFactory.getLogger(RoloClientConnector.class);

	protected ClientSession session;

	protected final CommandClientHandler handler;
	...
	public RoloClientConnector(String name, CommandClientHandler handler) {
		if (name == null) {
	...
	public boolean connect() {
		...
		try {
			...
			producer = session.createProducer("commandsQueue");
                                ...
                                while (!consumer.isClosed()) {
                                    try {
                                        ...
                                        if (serverMessage != null) {
                                            ((CommandClientHandler) handler).messageReceived(session, readMessage(serverMessage), "rolo-ui");
                                        }
                                    } catch (HornetQException e) {
                                        if (e.getCode() == HornetQException.OBJECT_CLOSED) {
                                        	try {
