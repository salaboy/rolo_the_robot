...
/**
 *
 * Based on processing serial, without depending on PApplet
 */
public class Serial implements SerialPortEventListener {
     ...
     */
    public Serial(Object parent, int irate) {
     ...
     */
    public Serial(Object parent, String iname, int irate) {
        this(parent, iname, irate, dparity, ddatabits, dstopbits);
    }

    public Serial(Object parent, String iname) {
        this(parent, iname, drate, dparity, ddatabits, dstopbits);
    }

    /**
     * @param iparity 'N' for none, 'E' for even, 'O' for odd ('N' is the
     * default)
...
//    // which would be called each time an event comes in
     ...
     * the very last value received and clears the buffer. Useful when you just
     * want the most recent value sent over the port.
     * @webref serial:serial
     * @usage web_application
     */
    public int last() {
     ...
     * output. Note that this doesn't account for Unicode (two bytes per char),
     * nor will it send UTF8 characters.. It assumes that you mean to send a
     * byte buffer (most often the case for networking and serial i/o) and will
     * only use the bottom 8 bits of each char in the string. (Meaning that
     ...
     * completes on Windows, it may be because the DLL doesn't have its exec bit
     * set. Why the hell that'd be the case, who knows.
     *
     * @webref serial
     * @usage web_application
     */
